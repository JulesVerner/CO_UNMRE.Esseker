...
_args params ["", "_duration", "_target", ["_height", 5], ["_radius", 10], ["_startingAngle", 0], ["_endAngle", 180], ["_fov", 0.75], ["_clockwise", true], ["_rise", 0]];
...
[
    {
        ...
        _args params ["_camera", "_target", "_angleDistance", "_endAngle", "_startTime", "_endTime", "_height", "_radius", "_clockwise"];
        
        private _remainingTime = _endTime - time; // eg.60
        

        if (time > _endTime || _remainingTime < 0.1) exitWith {
        ...
        if (!_clockwise) then { _progress = -_progress; };

        
        if (GRAD_introCam_camAngle < _endAngle && _clockwise) then {
    ...
    0,
    [_camera, _target, _angleDistance, _endAngle, _startTime, (time + _duration + 0.1), _height, _radius, _clockwise]
