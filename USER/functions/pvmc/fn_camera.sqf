...
private _bool = _player getVariable ["GRAD_USER_camerasActive", false];
if (_bool) then {
	{
		_x params ["_obj", "_index"];

		_obj setObjectTexture [_index, "#(argb,512,512,1)r2t(piprendertg6,1)"];
	...
	_player getVariable ["GRAD_USER_cameras", _cameras];
};
